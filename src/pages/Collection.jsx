...
export default function Collection() {
    ...
    const { plants, removePlant } = usePlantContext();
            ...
            <main className="flex-1 w-full max-w-[1000px] mx-auto px-6 py-12">
                ...
                <div className="flex flex-col gap-6">
                    {plants.map((plant, index) => (
                        <ScrollReveal key={plant.id} direction="up" delay={0.1 * (index + 1)}>
                            <div className="bg-surface p-6 rounded-lg flex flex-col md:flex-row items-center gap-6 hover:shadow-md transition-all duration-500 group relative overflow-hidden border border-outline-variant/30 cursor-pointer" onClick={() => navigate(`/plant/${plant.id}`)}>
                                ...
                                <div className="w-full md:w-32 h-32 flex-shrink-0 rounded-lg overflow-hidden border border-outline-variant/20 shadow-inner">
                                    <img alt={plant.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src={plant.image || "https://images.unsplash.com/photo-1596547609652-9fc5b8cb007c?q=80&w=400&auto=format&fit=crop"} />
                                </div>
                                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 w-full relative z-10 items-center">
                                    <div>
                                        <h3 className="font-headline-md text-[24px] font-semibold text-on-surface mb-1">{plant.name}</h3>
                                        <div className="flex items-center gap-2">
                                            <div className="w-2.5 h-2.5 rounded-full bg-primary"></div>
                                            <span className="font-label-md text-[14px] text-on-surface-variant font-medium">Health: {plant.status || 'Optimal'}</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col justify-center md:border-l border-outline-variant/30 md:pl-6 h-full min-h-[60px]">
                                        <p className="font-label-sm text-[11px] text-on-surface-variant uppercase tracking-widest mb-1">Species</p>
                                        <p className="font-body-lg text-[18px] text-on-surface font-semibold">{plant.species}</p>
                                    </div>
                                    <div className="flex items-center justify-start md:justify-end gap-2 mt-2 md:mt-0">
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); handleDelete(plant.id); }} className="p-2.5 w-10 h-10 flex items-center justify-center bg-error/10 text-error rounded-full hover:bg-error hover:text-white transition-all shadow-sm ml-1" title="Delete"><span className="material-symbols-outlined text-[20px]">delete</span></button>
                                    </div>
                                </div>
                            </div>
                        </ScrollReveal>
